...
class Queue
{
 ...
 /*------------------------------------------------------------------------
   Search through a queue for a particular key value, and if found,
   move the node to the front of the same queue.
   
   Precondition: Queue is nonempty.
   Postcondition: Value specified has been moved to the front of Queue.
 ------------------------------------------------------------------------*/
 
 void merge_two_queues(const Queue & queue2);
 /*------------------------------------------------------------------------
  Take two queues that are each ordered according to a given field,
  and merge the two queues into one queue whose absolute order of the
  merged elements is kept.
  
  Precondition: Both queues are nonempty.
  Postcondition: First queue contains all the merged, sorted elements and
                 queue2 is empty.
 ------------------------------------------------------------------------*/
