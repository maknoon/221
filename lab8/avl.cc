...
// ********** DO NOT CHANGE ABOVE HERE ****************
  ...
  //
void balance( Node *& x ) {
	int lh, rh, diff = 0; // difference b/t left->height & right->height
	Node* l = x->left; Node* r = x->right;
	
	// base case
	if (!x) return;
	
	// check if x is unbalanced by first finding diff between heights
	if (l || r) {
		if (!l) { lh = -1; rh = r->height; }
		else if (!r) { lh = l->height; rh = -1; }
		else { lh = l->height; rh = r->height; }

		diff = lh - rh;
	}
	
	// diff > 1 means left is heavier => all rotates are to the right
	if (diff > 1) {
		// only l->left is pulling down the tree, so rotate once
		if (l->left && (l->left->height+1 == lh)) rotateRight(x);
		// otherwise doubleRotate
		else doubleRotateRight(x);
	}
	// diff < -1 means right is heavier => all rotates are to the left
	if (diff < -1) {
		// only r->right is pulling down the tree, so rotate once
		if (r->right && (r->right->height+1 == rh)) rotateLeft(x);
		// otherwise doubleRotate
		else doubleRotateLeft(x);
	}
}
