...
#include <cmath>	// provides pow for calculating # nodes in AVL subtree
...
void findStatsHelper(Node * root, double &avg, double &var, int &count) {
	...
	// number nodes of an AVL subtree = 2^(height+1) - 1
	if (std::pow(2,(root->height+1))-1 > SEQUENTIAL_CUTOFF)
	{
		// referenced slide 23 from ss08
		// FORK: shared vars = lavg, lvar, lcount for left
...
#pragma omp taskwait
	...
	// #nodes <= SEQUENTIAL_CUTOFF :: same as before; do it sequentially
	else {
    	// Find stats in the left subtree.
    	findStatsSequential(root->left, lavg, lvar, lcount);
    	// Find stats in the right subtree.
...
void findStatsAndReport(std::function<void(Node*, double&, double&, int&)> f,
  ...
  std::cout << SEQUENTIAL_CUTOFF << ", type: " << type << ", avg: " << avg;
  std::cout << ", var: " << var << ", cnt: " << count << ", t: " << t2 - t1 << std::endl;
  std::cout.flush();
}
