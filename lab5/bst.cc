...
/*************************** MACROS ********************************/
// returns whichever is bigger, a or b
 ...
 * This function would change if the invariant was different.
 */
...
bool delete_node(Node*& root, int key) {
  ...
  // case 3: target has two children
    ...
    // so the left child of the deleted node takes the place
    // and then the left child's right node must point to the deleted node's
    // right child
    // to find the target's replacement:
    // (1) start from the left node because u want the key < target's key
    // (2) then find largest key value from that node by traversing right
    // (3) the predecessor is then the parent of the replacement
    Node* successor = target->left;
    while (successor->right) successor = successor->right;
    
    // if the target is already the parent of the replacement,
    // just link old left node to the old right node
    if (target == find_parent(root, successor)) successor->right = target->right;
    else {
      // otherwise
      // make sure that the predecessor still links any pre(x) < x < suc(x)
      // the sucessor's left should point to the target's left
      // and successor's right should point to target's right
      find_parent(root, successor)->right = successor->left;
      successor->right = target->right;
      successor->left = target->left;
    }
      
    // replace node as before
    if (parent != NULL) {
...
int numNodes( Node* root ) {
  // if we reached end of tree, go back w/ return 0
  // otherwise count the current node & keep traversing recursively
  if (root == NULL) return 0;
  else return (1 + numNodes(root->right) + numNodes(root->left));
}
...
int numLeaves( Node* root ) {
  ...
  if (root == NULL) return 0;
    ...
    // count the node if it doesnt have any children (= leaf)
    // otherwise keep traversing recursively to look for a leaf
...
int height( Node* x ) {
  ...
  if (x == NULL) return -1;
  // base case: if tree has only one node, height is 0
  else if (x->left == NULL && x->right == NULL) return 0;
  // height = the longest traversal path from root to leaf, so
  // we just want to compare the left and right nodes recursively,
  // and figure out which one is longer. then add longest to the height
...
int depth( Node* root, Node* x ) {
  ...
  if (root == NULL) return 0;
  // base case: if the x is the same as root, depth is still 0
  else if (root->key == x->key) return 0;
  else {
    // all keys in left subtree are < the desired key
    // so we don't need to go into the left subtree if current key < desired key
    if (root->right && x->key > root->key) return 1 + depth(root->right, x);
    // all keys in right subtree are > the desired key
    // so we don't need to go into the right subtree if current key > desired key
    if (root->left && root->key > x->key) return 1 + depth(root->left, x);
  }
  // if we can't find it we can just return 0
  return 0;
}
...
void in_order( Node* rootNode, int level, Visitor& v ) {
  ...
  // do the visits recursively based on the order
  if (rootNode->left) in_order(rootNode->left, level + 1, v);
  v.visit(rootNode, level);
  if (rootNode->right) in_order(rootNode->right, level + 1, v);
}
...
void pre_order( Node* rootNode, int level, Visitor& v ) {
  // pre order = visit root, visit left, visit right
  // do the visits recursively based on the order
  v.visit(rootNode, level);
  if (rootNode->left) pre_order(rootNode->left, level + 1, v);
  if (rootNode->right) pre_order(rootNode->right, level + 1, v);
}
...
void post_order( Node* rootNode, int level, Visitor& v ) {
  ...
  // do the visits recursively based on the order
  if (rootNode->left) post_order(rootNode->left, level + 1, v);
  if (rootNode->right) post_order(rootNode->right, level + 1, v);
  v.visit(rootNode, level);
}
