...
class max_heap
{
 ...
 public:
  ...
  /*** HELPER FUNCTIONS USED FOR TESTING ONLY ***/
  
  /*-----------------------------------------------------------------------
    A helper function. Prints current state of the heap

    Precondition:  heap is non-empty with size > 0
    Postcondition: heap is printed & heap invariant holds
  -----------------------------------------------------------------------*/
  void print();
  
  /*-----------------------------------------------------------------------
    A helper function. Prints heap in form of a binary tree uw u

    Precondition:  heap is non-empty with size > 0
    Postcondition: heap is printed & heap invariant holds
  -----------------------------------------------------------------------*/
  void bin_print(int node, int d);
