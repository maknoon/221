...
class string_bst
{
 public:
  ...
  /*-----------------------------------------------------------------------
    Returns a pointer to the node specified by key
    
    Precondition:  Tree is nonempty.
    Postcondition: Tree has not been modified, pointer to specified key's node
      has been returned.
  -----------------------------------------------------------------------*/
  node_t* find_word(string_bst::node_t* node, tree_key const& key) const;  
