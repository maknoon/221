...
// for diff functions w/ same name
#define PART_ONE
//#define PART_TWO
//#define DEBUG
...
void printHeap(int *heap, int size, int node=0, int d=0) {
    if (node >= size) return;
    // base case: print left and right branches of each node in heap
    if (node == 0) {
	cout << heap[node] << endl;
	printHeap(heap, size, node+1, d+1);
	printHeap(heap, size, node+2, d+1);
    } else {
        // print out the depth in asterisks, then print node
	for (int i = 0; i < d; i++) cout << "*";
	cout << heap[node] << endl;
	// determine child node/depth and print left and right branches
...
void printHeap(int *heap, int size, int node=0, int d=0) {
    if (node >= size) return;
    // base case: print left and right branches of each node in heap
    // (rearranged from part one to look like a tree)
    if (node == 0) {
	printHeap(heap, size, node+2, d+1);
	cout << heap[node] << endl;
	printHeap(heap, size, node+1, d+1);
    } else {
	// determine child node/depth and print right branch
	printHeap(heap, size, 2*node+2, d+1);
	// print out the depth in spaces, then print node
	for (int i = 0; i < d; i++) cout << "  ";
	cout << heap[node] << endl;
	// determine child node/depth and print left branch
	printHeap(heap, size, 2*node+1, d+1);
    }
}
#endif
...
/* ASYMPTOTIC ETC
 * Asymptotic time complexity is linear O(size): 
 * Function is recursive and goes down by size-1 each time, therefore
 * it is linear as it must go thru each element at least once.
 */
...
void remove(int* heap, int key, int & size) {
	...
	if (heap[i] == key) {
            heap[i] = heap[i+1];
	    size--;
	    remove(heap, key, size); // clean up rest of array
        }
    }
}
...
/* ASYMPTOTIC ETC - of the second solution, not debug one cuz it dumb
 * Asymptotic time complexity is O(size1+size2):
 * first for loop O1 = O(size1), second for loop O2 = O(size2)
 * but heapify is O3 = O(size1+size2) so we have max(O1, O2, O3) = O3
 */
...
#ifdef DEBUG
    // basically just add an element from each array at a time
    // we're gonna have to heapify it after anyway
    int i = 0;
    int j = 0;
    while (i < newSize) {
	if (j < size1) { newHeap[i] = heap1[j]; i++; }
	if (j < size2) { newHeap[i] = heap2[j]; i++; }
	j++;
    }
#endif
#ifndef DEBUG
    // or just concatenate heap2 to heap1 lol
    for (int i = 0; i < size1; i++) { newHeap[i] = heap1[i]; }
    for (int i = 0; i < size2; i++) { newHeap[i+size1] = heap2[i]; }
#endif
